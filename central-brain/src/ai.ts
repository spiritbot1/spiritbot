...
import { getToolDefinitions, executeTool, ALL_TOOLS } from './tools';
...
// 任务类型
type TaskType = 'light' | 'standard' | 'powerful' | 'reasoning' | 'coding';

/**
 * 智能分析问题，选择最合适的模型
 */
function analyzeTaskType(content: string): { type: TaskType; reason: string } {
...
function getModelForTask(taskType: TaskType): string {
  const models = config.ai.models as Record<string, string>;
  return models[taskType] || config.ai.defaultModel;
}
...
function buildToolPrompt(): string {
  ...
  const toolList = tools.map(t => {
    const func = (t as any).function;
    const params = func.parameters.properties;
    const paramStr = Object.entries(params)
      .map(([k, v]: [string, any]) => `    - ${k}: ${v.description || v.type}`)
      .join('\n');
...
export async function chatWithTools(
  ...
  try {
    ...
    let responseText: string = content;
    ...
    if (chineseMatch) {
      ...
      if (xmlThinking) thinkingText = xmlThinking[1].trim();
      if (xmlResponse) responseText = xmlResponse[1].trim();
    }
 ...
 * 自主思考
 */
export async function think(context: string): Promise<ThinkingResult> {
 ...
 * 生成好奇问题
 */
export async function generateCuriosityQuestions(topic: string): Promise<string[]> {
 ...
 * 分析和总结
 */
export async function analyze(content: string, instruction: string): Promise<string> {
 ...
 * 检测知识空白
 */
export async function detectKnowledgeGaps(recentTopics: string[]): Promise<string[]> {
