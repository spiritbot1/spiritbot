...
import { getConsciousnessState, triggerConsciousnessStep } from './consciousness-loop';
import * as kg from './knowledge-graph';
import * as lp from './learning-planner';
import * as ee from './evolution-evaluator';
import * as mp from './memory-pyramid';
import * as ec from './external-connector';
import { optionalApiKey, recordApiUsage, requirePermission, PUBLIC_ENDPOINTS } from './api-key-middleware';
...
// API Key 验证中间件 - 除公开端点外都需要验证
app.use((req, res, next) => {
  // 公开端点不需要 API Key
  if (PUBLIC_ENDPOINTS.includes(req.path)) {
    return next();
  }
  // 其他所有端点都需要有效的 API Key
...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    const enhancedMessages = dynamicPersonality
      ? [{ role: 'system' as const, content: dynamicPersonality }, ...messages]
      : messages;
...
app.get('/api/entities', async (req, res) => {
  try {
    ...
    const entities = await kg.searchEntities(
      query as string,
      type as kg.EntityType | undefined,
...
app.get('/api/memory/search', async (req, res) => {
  try {
    ...
    const memories = await mp.searchMemories(
      query as string,
      level as mp.MemoryLevel | undefined,
...
app.post('/api/consciousness/step', async (req, res) => {
  try {
    await triggerConsciousnessStep();
...
export function startApiServer(): void {
  app.listen(config.server.port, () => {
    console.log(`🌐 API 服务启动: http://localhost:${config.server.port}`);
    console.log('');
    console.log('可用接口:');
