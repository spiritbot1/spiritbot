...
exports.startConsciousnessLoop = startConsciousnessLoop;
exports.getConsciousnessState = getConsciousnessState;
exports.triggerConsciousnessStep = triggerConsciousnessStep;
...
async function perceive() {
    ...
    const { data: recentSessions } = await database
        ...
        .limit(10);
    const recentTopics = (recentSessions || []).map((s) => s.title);
    // 获取系统状态
 ...
 * 思考阶段 - 分析和形成想法
 */
async function think(perception) {
    console.log('🧠 思考阶段...');
    const context = `
 ...
 * 决策阶段 - 确定行动
 */
async function decide(thinking) {
    console.log('⚖️  决策阶段...');
    const actions = [];
 ...
 * 反思阶段 - 评估和学习
 */
async function reflect(thinking, results) {
    console.log('🪞 反思阶段...');
    // 保存学习记录
...
async function evolve() {
    console.log('🌱 进化阶段...');
    // 如果启用了好奇心引擎，生成新的探索方向
    if (config_1.config.consciousness.enableCuriosity && state.cycleCount % 5 === 0) {
        console.log('   🔮 运行好奇心引擎...');
...
function startConsciousnessLoop() {
    ...
    console.log(`   间隔: 每 ${config_1.config.consciousness.intervalMinutes} 分钟`);
    console.log(`   自主学习: ${config_1.config.consciousness.enableLearning ? '启用' : '禁用'}`);
    console.log(`   好奇心引擎: ${config_1.config.consciousness.enableCuriosity ? '启用' : '禁用'}`);
    // 立即运行一次
    runCycle();
    // 设置定时任务
    const cronExpression = `*/${config_1.config.consciousness.intervalMinutes} * * * *`;
 ...
 */
async function triggerConsciousnessStep() {
