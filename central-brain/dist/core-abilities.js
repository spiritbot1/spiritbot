...
exports.ABILITY_MANIFEST = exports.CORE_ABILITIES = void 0;
exports.searchWeb = searchWeb;
exports.readWebPage = readWebPage;
exports.analyzeAndPlan = analyzeAndPlan;
exports.makeDecision = makeDecision;
exports.writeDocument = writeDocument;
exports.writeCode = writeCode;
exports.callAPI = callAPI;
exports.exploreSystem = exploreSystem;
exports.sendMCPMessage = sendMCPMessage;
exports.analyzeData = analyzeData;
exports.learnKnowledge = learnKnowledge;
exports.recallKnowledge = recallKnowledge;
exports.reflectAndImprove = reflectAndImprove;
...
async function searchWeb(query) {
    try {
        ...
        const results = data.RelatedTopics?.slice(0, 5).map((topic) => ({
 ...
 * 分析问题并制定计划
 */
async function analyzeAndPlan(problem) {
    // 这是大脑自己的思考能力，通过 AI 模型实现
    const { chat } = await Promise.resolve().then(() => __importStar(require('./ai')));
 ...
 * 写文档/方案
 */
async function writeDocument(topic, type, requirements) {
    const { chat } = await Promise.resolve().then(() => __importStar(require('./ai')));
    const typeNames = {
        report: '报告',
        plan: '方案',
 ...
 * 写代码
 */
async function writeCode(description, language, context) {
    const { chat } = await Promise.resolve().then(() => __importStar(require('./ai')));
    const response = await chat([{
            role: 'user',
 ...
 */
async function sendMCPMessage(server, method, params) {
    // MCP 协议实现
    try {
        const message = {
            jsonrpc: '2.0',
 ...
 * 学习并记住新知识
 */
async function learnKnowledge(topic, content, source) {
    const { saveMemory } = await Promise.resolve().then(() => __importStar(require('./memory-pyramid')));
    const { learnFromText } = await Promise.resolve().then(() => __importStar(require('./knowledge-graph')));
    try {
...
async function recallKnowledge(query) {
    const { recall } = await Promise.resolve().then(() => __importStar(require('./memory-pyramid')));
    const { searchEntities } = await Promise.resolve().then(() => __importStar(require('./knowledge-graph')));
    try {
 ...
 * 反思并改进
 */
async function reflectAndImprove(action, result, wasSuccessful) {
    const { chat } = await Promise.resolve().then(() => __importStar(require('./ai')));
    const { saveMemory } = await Promise.resolve().then(() => __importStar(require('./memory-pyramid')));
    const response = await chat([{
            role: 'user',
