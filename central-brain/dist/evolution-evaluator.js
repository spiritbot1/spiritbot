...
exports.evaluateAllCapabilities = evaluateAllCapabilities;
exports.generateGrowthReport = generateGrowthReport;
exports.adjustLearningStrategy = adjustLearningStrategy;
 ...
 * 评估知识广度
 */
async function evaluateKnowledgeBreadth() {
    ...
    // 综合评分
    const score = Math.min(100, ((stats.totalEntities / 100) * 30 + // 实体数量
 ...
 * 评估知识深度
 */
async function evaluateKnowledgeDepth() {
    const stats = await kg.getGraphStats();
    // 关系密度 = 关系数 / 实体数
    ...
    // 综合评分
    const score = Math.min(100, (Math.min(relationDensity * 50, 50) + // 关系密度
 ...
 * 评估学习速度
 */
async function evaluateLearningSpeed() {
    const db = (0, database_1.getDatabase)();
    // 获取最近一周的学习记录
    ...
    const { count: newKnowledge } = await db
        ...
        .gte('created_at', oneWeekAgo.toISOString());
    // 评分：基于学习频率和知识产出
    const score = Math.min(100, (learningCount * 10 +
        (newKnowledge || 0) * 5));
    return {
...
async function evaluateAutonomy() {
    ...
    const { count: curiosityTasks } = await db
        ...
        .eq('type', 'curiosity');
    // 评分
    const score = Math.min(100, ((selfCreatedTasks || 0) * 5 +
        (curiosityTasks || 0) * 10));
    return {
        dimension: 'autonomy',
        score: Math.round(score),
  ...
  "self_score": 分数
    ...
    try {
        ...
        if (jsonMatch) {
            ...
            // 根据答案正确性调整
            if (result.answer1 === correctAnswer1)
                score += 10;
            if (result.answer2 === correctAnswer2)
                score += 10;
            return {
                dimension: 'reasoning',
 ...
 * 评估自我认知
 */
async function evaluateSelfAwareness() {
  ...
  "self_score": 分数
    ...
    try {
        ...
        if (jsonMatch) {
            const result = JSON.parse(jsonMatch[0]);
            // 检查是否有实质内容（不是空话）
            const hasSubstance = result.knowledge_gaps?.length > 10 &&
 ...
 * 执行全面能力评估
 */
async function evaluateAllCapabilities() {
    console.log('📊 开始能力评估...');
 ...
 * 生成成长报告
 */
async function generateGrowthReport(days = 7) {
    ...
    // 生成成就
    const achievements = [];
    if ((newKnowledge || 0) > 10)
        achievements.push(`学习了 ${newKnowledge} 个新知识点`);
    if ((conversations || 0) > 5)
        achievements.push(`进行了 ${conversations} 次对话`);
    if (currentEval.overall_score > 60)
        achievements.push('综合能力评分超过 60 分');
    // 待改进领域
 ...
 * 根据评估结果调整学习策略
 */
async function adjustLearningStrategy(evaluation) {
    const adjustments = [];
    ...
    if (adjustments.length > 0) {
        const db = (0, database_1.getDatabase)();
        await db.from('central_brain_tasks').insert(adjustments.map((adj, i) => ({
