...
function createProfileContext(opts, profile) {
    ...
    const ensureBrowserAvailable = async () => {
        ...
        if (isExtension && remoteCdp) {
            throw new Error(`Profile "${profile.name}" uses driver=extension but cdpUrl is not loopback (${profile.cdpUrl}).`);
        }
        if (isExtension) {
            if (!httpReachable) {
                ...
                if (await isHttpReachable(1200)) {
                    ...
                    throw new Error(`Chrome extension relay for profile "${profile.name}" is not reachable at ${profile.cdpUrl}.`);
                }
            }
        ...
        if (!profileState.running) {
            throw new Error(`Port ${profile.cdpPort} is in use for profile "${profile.name}" but not by moltbot. ` +
