...
async function waitForGeminiBatch(params) {
    ...
    while (true) {
        ...
        if (["SUCCEEDED", "COMPLETED", "DONE"].includes(state)) {
            const outputFileId = status.outputConfig?.file ??
...
export async function runGeminiEmbeddingBatches(params) {
    ...
    const tasks = groups.map((group, groupIndex) => async () => {
        ...
        if (!params.wait &&
            ...
            throw new Error(`gemini batch ${batchName} submitted; enable remote.batch.wait to await completion`);
        }
        const completed = batchInfo.state && ["SUCCEEDED", "COMPLETED", "DONE"].includes(batchInfo.state)
            ? {
                outputFileId: batchInfo.outputConfig?.file ??
