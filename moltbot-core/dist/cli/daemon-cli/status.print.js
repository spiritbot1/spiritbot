...
export function printDaemonStatus(status, opts) {
    ...
    if (service.configAudit?.issues.length) {
        ...
        defaultRuntime.error(warnText(`Recommendation: run "${formatCliCommand("moltbot doctor")}" (or "${formatCliCommand("moltbot doctor --repair")}").`));
    }
    if (status.config) {
        ...
        if (!status.config.cli.valid && status.config.cli.issues?.length) {
            for (const issue of status.config.cli.issues.slice(0, 5)) {
                defaultRuntime.error(`${errorText("Config issue:")} ${issue.path || "<root>"}: ${issue.message}`);
            }
        }
        if (status.config.daemon) {
            ...
            if (!status.config.daemon.valid && status.config.daemon.issues?.length) {
                for (const issue of status.config.daemon.issues.slice(0, 5)) {
                    defaultRuntime.error(`${errorText("Service config issue:")} ${issue.path || "<root>"}: ${issue.message}`);
                }
            }
        }
        if (status.config.mismatch) {
            ...
            defaultRuntime.error(errorText(`Fix: rerun \`${formatCliCommand("moltbot gateway install --force")}\` from the same --profile / CLAWDBOT_STATE_DIR you expect.`));
        }
        spacer();
    }
    if (status.gateway) {
    ...
    if (runtimeLine) {
        ...
        const runtimeColor = runtimeStatus === "running"
            ? theme.success
    ...
    if (service.runtime?.cachedLabel) {
        ...
        defaultRuntime.error(errorText(`Then reinstall: ${formatCliCommand("moltbot gateway install")}`));
        spacer();
    }
    ...
    if (status.portCli && status.portCli.port !== status.port?.port) {
        defaultRuntime.log(`${label("Note:")} CLI config resolves gateway port=${status.portCli.port} (${status.portCli.status}).`);
    }
    if (service.loaded &&
