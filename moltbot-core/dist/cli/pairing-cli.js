...
export function registerPairingCli(program) {
    ...
    pairing
        ...
        if (!opts.channel && !code) {
            throw new Error(`Usage: ${formatCliCommand("moltbot pairing approve <channel> <code>")} (or: ${formatCliCommand("moltbot pairing approve --channel <channel> <code>")})`);
        }
        if (opts.channel && code != null) {
            throw new Error(`Too many arguments. Use: ${formatCliCommand("moltbot pairing approve --channel <channel> <code>")}`);
        }
        const channel = parseChannel(channelRaw, channels);
