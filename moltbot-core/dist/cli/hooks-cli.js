...
export function formatHookInfo(report, hookName, opts) {
    ...
    // Requirements
    const hasRequirements = hook.requirements.bins.length > 0 ||
    ...
    if (hasRequirements) {
        ...
        if (hook.requirements.anyBins.length > 0) {
            const anyBinsStatus = hook.missing.anyBins.length > 0
        ...
        if (hook.requirements.os.length > 0) {
            const osStatus = hook.missing.os.length > 0
...
export async function enableHook(hookName) {
    ...
    if (hook.managedByPlugin) {
        throw new Error(`Hook "${hookName}" is managed by plugin "${hook.pluginId ?? "unknown"}" and cannot be enabled/disabled.`);
    }
    if (!hook.eligible) {
