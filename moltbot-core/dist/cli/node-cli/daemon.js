...
import { resolveNodeLaunchAgentLabel, resolveNodeSystemdServiceName, resolveNodeWindowsTaskName, } from "../../daemon/constants.js";
...
export async function runNodeDaemonInstall(opts) {
...
export async function runNodeDaemonUninstall(opts = {}) {
    const json = Boolean(opts.json);
    const stdout = json ? createNullWriter() : process.stdout;
    const emit = (payload) => {
        if (!json)
...
export async function runNodeDaemonStart(opts = {}) {
    const json = Boolean(opts.json);
    const stdout = json ? createNullWriter() : process.stdout;
    const emit = (payload) => {
        if (!json)
...
export async function runNodeDaemonRestart(opts = {}) {
    const json = Boolean(opts.json);
    const stdout = json ? createNullWriter() : process.stdout;
    const emit = (payload) => {
        if (!json)
...
export async function runNodeDaemonStop(opts = {}) {
    const json = Boolean(opts.json);
    const stdout = json ? createNullWriter() : process.stdout;
    const emit = (payload) => {
        if (!json)
...
export async function runNodeDaemonStatus(opts = {}) {
    const json = Boolean(opts.json);
    const service = resolveNodeService();
