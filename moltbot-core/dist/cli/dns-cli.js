...
export function registerDnsCli(program) {
    ...
    dns
        ...
        if (cfg.discovery?.wideArea?.enabled !== true) {
            defaultRuntime.log("");
            defaultRuntime.log(theme.muted("Note: enable discovery.wideArea.enabled in ~/.clawdbot/moltbot.json on the gateway and restart the gateway so it writes the DNS-SD zone."));
        }
    });
}
