...
function normalizeTag(value) {
    ...
    if (!trimmed)
        return null;
    if (trimmed.startsWith("moltbot@"))
        return trimmed.slice("moltbot@".length);
...
async function ensureGitCheckout(params) {
    ...
    if (!(await isGitCheckout(params.dir))) {
        ...
        if (!empty) {
            throw new Error(`CLAWDBOT_GIT_DIR points at a non-git directory: ${params.dir}. Set CLAWDBOT_GIT_DIR to an empty folder or a moltbot checkout.`);
        }
        return await runUpdateStep({
            name: "git clone",
            argv: ["git", "clone", CLAWDBOT_REPO_URL, params.dir],
...
export async function updateCommand(opts) {
    ...
    if (result.status === "skipped") {
        ...
        if (result.reason === "not-git-install") {
            defaultRuntime.log(theme.warn(`Skipped: this Moltbot install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("moltbot doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("moltbot gateway restart"), CLI_NAME)}\`.`));
            defaultRuntime.log(theme.muted(`Examples: \`${replaceCliName("npm i -g moltbot@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g moltbot@latest", CLI_NAME)}\``));
        }
        defaultRuntime.exit(0);
        return;
    }
    ...
    if (shouldRestart) {
        ...
        try {
            ...
            if (!opts.json) {
                ...
                defaultRuntime.log(theme.muted(`You may need to restart the service manually: ${replaceCliName(formatCliCommand("moltbot gateway restart"), CLI_NAME)}`));
            }
        }
    }
    else if (!opts.json) {
        ...
        if (result.mode === "npm" || result.mode === "pnpm") {
            defaultRuntime.log(theme.muted(`Tip: Run \`${replaceCliName(formatCliCommand("moltbot doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("moltbot gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`));
        }
        else {
            defaultRuntime.log(theme.muted(`Tip: Run \`${replaceCliName(formatCliCommand("moltbot gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`));
        }
    }
    if (!opts.json) {
    ...
    if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
        ...
        if (!hasGit) {
            ...
            if (dirExists) {
                ...
                if (!empty) {
                    defaultRuntime.error(`CLAWDBOT_GIT_DIR points at a non-git directory: ${gitDir}. Set CLAWDBOT_GIT_DIR to an empty folder or a moltbot checkout.`);
                    defaultRuntime.exit(1);
                    return;
                }
            }
