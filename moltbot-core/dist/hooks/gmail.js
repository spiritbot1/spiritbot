...
export function resolveGmailHookRuntimeConfig(cfg, overrides) {
    ...
    const servePort = typeof servePortRaw === "number" && Number.isFinite(servePortRaw) && servePortRaw > 0
        ? Math.floor(servePortRaw)
        : DEFAULT_GMAIL_SERVE_PORT;
    const servePathRaw = overrides.servePath ?? gmail?.serve?.path;
    const normalizedServePathRaw = typeof servePathRaw === "string" && servePathRaw.trim().length > 0
    ...
    const tailscaleTarget = tailscaleMode !== "off" &&
    ...
    const tailscalePath = normalizeServePath(tailscaleMode !== "off"
