...
export async function monitorSignalProvider(opts = {}) {
    ...
    const historyLimit = Math.max(0, accountInfo.config.historyLimit ??
    ...
    const startupTimeoutMs = Math.min(120_000, Math.max(1_000, opts.startupTimeoutMs ?? accountInfo.config.startupTimeoutMs ?? 30_000));
