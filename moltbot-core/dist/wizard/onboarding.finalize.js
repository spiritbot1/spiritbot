...
import { detectBrowserOpenSupport, formatControlUiSshHint, openUrl, openUrlInBackground, probeGatewayReachable, waitForGatewayReachable, resolveControlUiLinks, } from "../commands/onboard-helpers.js";
...
export async function finalizeOnboardingWizard(options) {
    ...
    if (installDaemon) {
        const daemonRuntime = flow === "quickstart"
        ...
        if (flow === "quickstart") {
            await prompter.note("QuickStart uses Node for the Gateway service (stable + supported).", "Gateway service runtime");
        }
        ...
        if (loaded) {
            ...
            if (action === "restart") {
                await withWizardProgress("Gateway service", { doneMessage: "Gateway service restarted." }, async (progress) => {
                ...
                await withWizardProgress("Gateway service", { doneMessage: "Gateway service uninstalled." }, async (progress) => {
    ...
    const tokenParam = settings.authMode === "token" && settings.gatewayToken
    ...
    if (!opts.skipUi && gatewayProbe.ok) {
        ...
        if (hatchChoice === "tui") {
            ...
            if (seededInBackground) {
                await prompter.note(`Web UI seeded in the background. Open later with: ${formatCliCommand("moltbot dashboard --no-open")}`, "Web UI");
            }
        }
            ...
            await prompter.note(`When you're ready: ${formatCliCommand("moltbot dashboard --no-open")}`, "Later");
        }
    }
    else if (opts.skipUi) {
        await prompter.note("Skipping Control UI/TUI prompts.", "Control UI");
    }
    await prompter.note(["Back up your agent workspace.", "Docs: https://docs.molt.bot/concepts/agent-workspace"].join("\n"), "Workspace backup");
    await prompter.note("Running agents on your computer is risky — harden your setup: https://docs.molt.bot/security", "Security");
    const shouldOpenControlUi = !opts.skipUi &&
