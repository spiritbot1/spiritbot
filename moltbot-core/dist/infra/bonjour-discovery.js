...
function parseTailscaleStatusIPv4s(stdout) {
    ...
    if (peerObj && typeof peerObj === "object") {
        for (const peer of Object.values(peerObj)) {
            addIps(peer);
        }
    }
    return [...new Set(out)];
...
function parseDnsSdBrowse(stdout) {
    ...
    for (const raw of stdout.split("\n")) {
        const line = raw.trim();
        if (!line || !line.includes("_moltbot-gw._tcp"))
            continue;
...
export async function discoverGatewayBeacons(opts = {}) {
    ...
    try {
        ...
        if (platform === "linux") {
            const perDomain = await Promise.allSettled(domains.map(async (domain) => await discoverViaAvahi(domain, timeoutMs, run)));
