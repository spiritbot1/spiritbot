    ...
    if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
        ...
        if (channel === "dev") {
            if (needsCheckoutMain) {
                const checkoutStep = await runStep(step(`git checkout ${DEV_BRANCH}`, ["git", "-C", gitRoot, "checkout", DEV_BRANCH], gitRoot));
            ...
            const checkoutStep = await runStep(step(`git checkout ${tag}`, ["git", "-C", gitRoot, "checkout", "--detach", tag], gitRoot));
            steps.push(checkoutStep);
            if (checkoutStep.exitCode !== 0) {
                return {
                    status: "error",
        ...
        const doctorStep = await runStep(step("moltbot doctor", managerScriptArgs(manager, "moltbot", ["doctor", "--non-interactive"]), gitRoot, { CLAWDBOT_UPDATE_IN_PROGRESS: "1" }));
