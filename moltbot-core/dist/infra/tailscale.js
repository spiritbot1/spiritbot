...
export async function ensureFunnel(port, exec = runExec, runtime = defaultRuntime, prompt = promptYesNo) {
    ...
    try {
        ...
        runtime.error(info(`Tip: Funnel is optional for Moltbot. You can keep running the web gateway without it: \`${formatCliCommand("moltbot gateway")}\``));
        if (shouldLogVerbose()) {
            const rich = isRich();
            if (stdout.trim()) {
