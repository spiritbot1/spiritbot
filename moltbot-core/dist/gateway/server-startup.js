...
export async function startGatewaySidecars(params) {
    ...
    if (params.cfg.hooks?.gmail?.model) {
        ...
        if (hooksModelRef) {
            ...
            if (!status.allowed) {
                params.logHooks.warn(`hooks.gmail.model "${status.key}" not in agents.defaults.models allowlist (will use primary instead)`);
            }
            if (!status.inCatalog) {
                params.logHooks.warn(`hooks.gmail.model "${status.key}" not in the model catalog (may fail at runtime)`);
            }
        }
    }
