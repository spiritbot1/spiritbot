...
export function createGatewayReloadHandlers(params) {
    const applyHotReload = async (plan, nextConfig) => {
        ...
        setCommandLaneConcurrency("cron" /* CommandLane.Cron */, nextConfig.cron?.maxConcurrentRuns ?? 1);
        setCommandLaneConcurrency("main" /* CommandLane.Main */, resolveAgentMaxConcurrent(nextConfig));
        setCommandLaneConcurrency("subagent" /* CommandLane.Subagent */, resolveSubagentMaxConcurrent(nextConfig));
