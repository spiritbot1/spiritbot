...
async function resolveThreadTsFromHistory(params) {
    try {
        ...
        if (shouldLogVerbose()) {
            logVerbose(`slack inbound: failed to resolve thread_ts via conversations.history for channel=${params.channelId} ts=${params.messageTs}: ${String(err)}`);
        }
        return undefined;
    }
}
export function createSlackThreadTsResolver(params) {
    ...
    return {
        resolve: async (request) => {
            ...
            if (shouldLogVerbose()) {
                logVerbose(`slack inbound: missing thread_ts for thread reply channel=${message.channel} ts=${message.ts} source=${request.source}`);
            }
            ...
            if (resolved) {
                if (shouldLogVerbose()) {
                    logVerbose(`slack inbound: resolved missing thread_ts channel=${message.channel} ts=${message.ts} -> thread_ts=${resolved}`);
                }
