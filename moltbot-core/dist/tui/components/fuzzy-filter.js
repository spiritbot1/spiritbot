 ...
 */
export function findWordBoundaryIndex(text, query) {
    if (!query)
        return null;
 ...
 */
export function fuzzyMatchLower(queryLower, textLower) {
    ...
    for (let i = 0; i < textLower.length && queryIndex < queryLower.length; i++) {
        if (textLower[i] === queryLower[queryIndex]) {
            ...
            if (lastMatchIndex === i - 1) {
                ...
                if (lastMatchIndex >= 0)
                    score += (i - lastMatchIndex - 1) * 2; // Penalize gaps
            }
            if (isAtWordBoundary)
 ...
 */
export function fuzzyFilterLower(items, queryLower) {
 ...
 */
export function prepareSearchItems(items) {
