import { resolveAuthProfileDisplayLabel, } from "./auth-profiles.js";
export const DEFAULT_OAUTH_WARN_MS = 24 * 60 * 60 * 1000;
export function resolveAuthProfileSource(_profileId) {
    return "store";
}
export function formatRemainingShort(remainingMs) {
...
function buildProfileHealth(params) {
    ...
    if (credential.type === "token") {
        const expiresAt = typeof credential.expires === "number" && Number.isFinite(credential.expires)
            ? credential.expires
            : undefined;
...
export function buildAuthHealthSummary(params) {
    ...
    const profiles = Object.entries(params.store.profiles)
        ...
        .map(([profileId, credential]) => buildProfileHealth({
        profileId,
        credential,
        store: params.store,
        cfg: params.cfg,
    ...
    const providers = Array.from(providersMap.values()).sort((a, b) => a.provider.localeCompare(b.provider));
