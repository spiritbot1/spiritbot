...
export async function handleDiscordMessagingAction(action, params, isActionEnabled) {
    const resolveChannelId = () => resolveDiscordChannelId(readStringParam(params, "channelId", {
        required: true,
    }));
    ...
    switch (action) {
        ...
        case "poll": {
            ...
            await sendPollDiscord(to, { question, options: answers, maxSelections, durationHours }, { ...(accountId ? { accountId } : {}), content });
            return jsonResult({ ok: true });
        }
        case "permissions": {
        ...
        case "threadCreate": {
            ...
            const thread = accountId
                ? await createThreadDiscord(channelId, { name, messageId, autoArchiveMinutes }, { accountId })
