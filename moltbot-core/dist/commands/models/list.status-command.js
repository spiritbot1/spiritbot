...
export async function modelsStatusCommand(opts, runtime) {
    ...
    const providersFromStore = new Set(Object.values(store.profiles)
        ...
        .filter((p) => Boolean(p)));
    const providersFromConfig = new Set(Object.keys(cfg.models?.providers ?? {})
        .map((p) => p.trim())
        .filter(Boolean));
    const providersFromModels = new Set();
    ...
    const providersFromEnv = new Set();
    ...
    const resolvedCandidates = rawCandidates
        .map((raw) => resolveModelRefFromString({
        raw: String(raw ?? ""),
        defaultProvider: DEFAULT_PROVIDER,
        aliasIndex,
    })?.ref)
    ...
    let probeSummary;
    if (opts.probe) {
        probeSummary = await withProgressTotals({ label: "Probing auth profiles…", total: 1 }, async (update) => {
    ...
    const providersWithOauth = providerAuth
        .filter((entry) => entry.profiles.oauth > 0 || entry.profiles.token > 0 || entry.env?.value === "OAuth (env)")
        .map((entry) => {
    ...
    for (const entry of providerAuth) {
        ...
        if (entry.env) {
            bits.push(formatKeyValue("env", `${entry.env.value}${separator}${formatKeyValue("source", entry.env.source)}`));
        }
        if (entry.modelsJson) {
            bits.push(formatKeyValue("models.json", `${entry.modelsJson.value}${separator}${formatKeyValue("source", entry.modelsJson.source)}`));
        }
    ...
    if (oauthProfiles.length === 0) {
        ...
        const usageProviders = Array.from(new Set(oauthProfiles
        ...
        for (const [provider, profiles] of profilesByProvider) {
            ...
            for (const profile of profiles) {
                ...
                const expiry = profile.status === "static"
                    ? ""
                    : profile.expiresAt
