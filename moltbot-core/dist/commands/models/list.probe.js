...
function buildProbeTargets(params) {
    ...
    return loadModelCatalog({ config: cfg }).then((catalog) => {
        ...
        for (const provider of providers) {
            ...
            const allowedProfiles = explicitOrder && explicitOrder.length > 0
...
export function formatProbeLatency(latencyMs) {
    if (!latencyMs && latencyMs !== 0)
        return "-";
    return formatMs(latencyMs);
}
export function groupProbeResults(results) {
    const map = new Map();
...
export function sortProbeResults(results) {
...
export function describeProbeSummary(summary) {
