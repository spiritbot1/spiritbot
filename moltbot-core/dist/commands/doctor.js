...
export async function doctorCommand(runtime = defaultRuntime, options = {}) {
    ...
    if (cfg.hooks?.gmail?.model?.trim()) {
        ...
        if (!hooksModelRef) {
            ...
            const warnings = [];
            if (!status.allowed) {
                warnings.push(`- hooks.gmail.model "${status.key}" not in agents.defaults.models allowlist (will use primary instead)`);
            }
            if (!status.inCatalog) {
                warnings.push(`- hooks.gmail.model "${status.key}" not in the model catalog (may fail at runtime)`);
            }
