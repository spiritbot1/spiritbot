...
import { applyWizardMetadata, DEFAULT_WORKSPACE, ensureWorkspaceAndSessions, guardCancel, printWizardHeader, probeGatewayReachable, resolveControlUiLinks, summarizeExistingConfig, waitForGatewayReachable, } from "./onboard-helpers.js";
...
async function promptWebToolsConfig(nextConfig, runtime) {
    ...
    if (enableSearch) {
        const keyInput = guardCancel(await text({
...
export async function runConfigureWizard(opts, runtime = defaultRuntime) {
    try {
        ...
        if (snapshot.exists) {
            ...
            if (!snapshot.valid) {
                outro(`Config invalid. Run \`${formatCliCommand("moltbot doctor")}\` to repair it, then re-run configure.`);
                runtime.exit(1);
                return;
            }
        }
        ...
        let workspaceDir = nextConfig.agents?.defaults?.workspace ??
