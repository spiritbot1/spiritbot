...
export const formatHealthChannelLines = (summary, opts = {}) => {
    ...
    for (const channelId of channelOrder) {
        ...
        const filteredSummaries = accountIds && accountIds.length > 0
            ? accountIds
                .map((accountId) => accountSummaries[accountId])
                .filter((entry) => Boolean(entry))
            : undefined;
        const listSummaries = accountMode === "all"
        ...
        const accountTimings = accountMode === "all"
            ? listSummaries
...
export async function getHealthSnapshot(params) {
    ...
    const heartbeatSeconds = defaultAgent?.heartbeat.everyMs
        ...
        : 0;
    const sessions = defaultAgent?.sessions ??
    ...
    for (const plugin of listChannelPlugins()) {
        ...
        const accountIdsToProbe = Array.from(new Set([preferredAccountId, defaultAccountId, ...accountIds, ...boundAccountIdsAll].filter((value) => value && value.trim())));
        debugHealth("channel", {
            id: plugin.id,
            accountIds,
            defaultAccountId,
        ...
        for (const accountId of accountIdsToProbe) {
            ...
            const bot = probeRecord && typeof probeRecord.bot === "object"
                ? probeRecord.bot
                : null;
            if (bot?.username) {
        ...
        const defaultSummary = accountSummaries[preferredAccountId] ??
