...
export async function noteSecurityWarnings(cfg) {
    ...
    const bindMode = bindModes.includes(gatewayBind)
        ? gatewayBind
        : undefined;
    ...
    if (isExposed) {
        if (!hasSharedSecret) {
            const authFixLines = resolvedAuth.mode === "password"
                ? [
                ...
                : [
                    ...
                    `  Or set token directly: ${formatCliCommand("moltbot config set gateway.auth.mode token")}`,
                ];
            warnings.push(`- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`, `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`, `  Fix: ${formatCliCommand("moltbot config set gateway.bind loopback")}`, ...authFixLines);
        }
        else {
