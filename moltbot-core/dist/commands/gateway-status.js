...
import { buildNetworkHints, extractConfigSummary, parseTimeoutMs, pickGatewaySelfPresence, renderProbeSummaryLine, renderTargetHeader, resolveAuthForTarget, resolveProbeBudgetMs, resolveTargets, sanitizeSshTarget, } from "./gateway-status/helpers.js";
export async function gatewayStatusCommand(opts, runtime) {
    ...
    const primary = reachable.find((p) => p.target.kind === "explicit") ??
    ...
    for (const p of probed) {
        ...
        if (p.configSummary) {
            ...
            const wideArea = c.discovery.wideAreaEnabled === true
                ? "enabled"
