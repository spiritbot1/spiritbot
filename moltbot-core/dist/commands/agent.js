...
export async function agentCommand(opts, runtime = defaultRuntime, deps = createDefaultDeps()) {
    ...
    if (agentIdOverride) {
        ...
        if (!knownAgents.includes(agentIdOverride)) {
            throw new Error(`Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("moltbot agents list")}" to see configured agents.`);
        }
    }
    ...
    const { sessionId, sessionKey, sessionEntry: resolvedSessionEntry, sessionStore, storePath, isNewSession, persistedThinking, persistedVerbose, } = sessionResolution;
    ...
    try {
        ...
        let resolvedThinkLevel = thinkOnce ??
            thinkOverride ??
            persistedThinking ??
