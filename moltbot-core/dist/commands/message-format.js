...
export function buildMessageCliJson(result) {
    return {
...
function renderMessageList(messages, opts, emptyLabel) {
    const rows = messages.slice(0, 25).map((m) => {
        ...
        const author = (typeof msg.authorTag === "string" && msg.authorTag) ||
            (typeof authorObj?.username === "string" && authorObj.username) ||
            (typeof msg.user === "string" && msg.user) ||
            "";
        const time = (typeof msg.timestamp === "string" && msg.timestamp) ||
            (typeof msg.ts === "string" && msg.ts) ||
            "";
...
function renderReactions(payload, opts) {
    ...
    const rows = reactions.slice(0, 50).map((r) => {
        ...
        const emoji = (typeof emojiObj?.raw === "string" && emojiObj.raw) ||
...
export function formatMessageCliText(result) {
    const rich = isRich();
