...
import { isLaunchAgentListed, isLaunchAgentLoaded, launchAgentPlistExists, repairLaunchAgentBootstrap, } from "../daemon/launchd.js";
...
export async function maybeRepairGatewayDaemon(params) {
    ...
    if (process.platform === "darwin") {
        ...
        note(`LaunchAgent loaded; stopping requires "${formatCliCommand("moltbot gateway stop")}" or launchctl bootout gui/$UID/${label}.`, "Gateway");
    }
    if (serviceRuntime?.status === "running") {
