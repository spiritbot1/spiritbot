...
export async function agentsAddCommand(opts, runtime = defaultRuntime, params) {
    ...
    if (nonInteractive) {
        ...
        const bindingResult = bindingParse.bindings.length > 0
        ...
        if (opts.json) {
            ...
            if (bindingResult.conflicts.length > 0) {
                runtime.error([
                    ...
                    ...bindingResult.conflicts.map((conflict) => `- ${describeBinding(conflict.binding)} (agent=${conflict.existingAgentId})`),
                ].join("\n"));
            }
        }
        return;
        ...
        if (selection.length > 0) {
            ...
            if (wantsBindings) {
                ...
                if (result.conflicts.length > 0) {
                    await prompter.note([
                        "Skipped bindings already claimed by another agent:",
                        ...result.conflicts.map((conflict) => `- ${describeBinding(conflict.binding)} (agent=${conflict.existingAgentId})`),
                    ].join("\n"), "Routing bindings");
                }
            }
            else {
