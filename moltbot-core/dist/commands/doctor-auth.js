...
import { CLAUDE_CLI_PROFILE_ID, CODEX_CLI_PROFILE_ID, ensureAuthProfileStore, repairOAuthProfileIdMismatch, resolveApiKeyForProfile, resolveProfileUnusableUntilForDisplay, } from "../agents/auth-profiles.js";
import { updateAuthProfileStoreWithLock } from "../agents/auth-profiles/store.js";
import { note } from "../terminal/note.js";
import { formatCliCommand } from "../cli/command-format.js";
export async function maybeRepairAnthropicOAuthProfileId(cfg, prompter) {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(cfg, prompter) {
    ...
    if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
        lines.push(`- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("moltbot models auth setup-token")}`);
    }
    if (deprecated.has(CODEX_CLI_PROFILE_ID)) {
        lines.push(`- ${CODEX_CLI_PROFILE_ID} (OpenAI Codex): use OAuth → ${formatCliCommand("moltbot models auth login --provider openai-codex")}`);
    }
    note(lines.join("\n"), "Auth profiles");
    ...
    if (pruned.changed) {
        note(Array.from(deprecated.values())
...
function formatAuthIssueHint(issue) {
    if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
        return `Deprecated profile. Use ${formatCliCommand("moltbot models auth setup-token")} or ${formatCliCommand("moltbot configure")}.`;
    }
    if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
        return `Deprecated profile. Use ${formatCliCommand("moltbot models auth login --provider openai-codex")} or ${formatCliCommand("moltbot configure")}.`;
    }
...
export async function noteAuthProfileHealth(params) {
    ...
    const unusable = (() => {
        ...
        for (const profileId of Object.keys(store.usageStats ?? {})) {
            ...
            const kind = typeof stats?.disabledUntil === "number" && now < stats.disabledUntil
    ...
    const findIssues = () => summary.profiles.filter((profile) => (profile.type === "oauth" || profile.type === "token") &&
