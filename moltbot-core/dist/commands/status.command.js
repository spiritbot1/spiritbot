...
import { resolveMemoryCacheSummary, resolveMemoryFtsState, resolveMemoryVectorState, } from "../memory/status-format.js";
import { formatHealthChannelLines } from "./health.js";
import { resolveControlUiLinks } from "./onboard-helpers.js";
import { getDaemonStatusSummary, getNodeDaemonStatusSummary } from "./status.daemon.js";
import { formatAge, formatDuration, formatKTokens, formatTokensCompact, shortenText, } from "./status.format.js";
...
export async function statusCommand(opts, runtime) {
    ...
    const scan = await scanStatus({ json: opts.json, timeoutMs: opts.timeoutMs, all: opts.all }, runtime);
    ...
    const rich = true;
    const muted = (value) => (rich ? theme.muted(value) : value);
    const ok = (value) => (rich ? theme.success(value) : value);
    ...
    const gatewayValue = (() => {
        ...
        const auth = gatewayReachable && !remoteUrlMissing
    ...
    const agentsValue = (() => {
        const pending = agentStatus.bootstrapPendingCount > 0
    ...
    const storeLabel = summary.sessions.paths.length > 1
