...
async function collectChannelStatus(params) {
    ...
    const catalogEntries = listChannelPluginCatalogEntries({ workspaceDir }).filter((entry) => !installedIds.has(entry.id));
...
export async function setupChannels(cfg, runtime, prompter, options) {
    let next = cfg;
    ...
    const { installedPlugins, catalogEntries, statusByChannel, statusLines } = await collectChannelStatus({ cfg: next, options, accountOverrides });
    ...
    const accountIdsByChannel = new Map();
    ...
    const resolveDisabledHint = (channel) => {
        ...
        else if (typeof account?.enabled === "boolean") {
            enabled = account.enabled;
        }
        else if (typeof next.channels?.[channel]
            ?.enabled === "boolean") {
            enabled = next.channels[channel]?.enabled;
        }
    ...
    const handleConfiguredChannel = async (channel, label) => {
        ...
        const shouldPromptAccount = action === "delete"
