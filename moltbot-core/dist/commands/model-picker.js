...
export async function promptDefaultModel(params) {
    ...
    if (allowKeep) {
        options.push({
            ...
            hint: configuredRaw && configuredRaw !== resolvedKey ? `resolves to ${resolvedKey}` : undefined,
        });
    }
    if (includeManual) {
    ...
    let initialValue = allowKeep ? KEEP_VALUE : configuredKey || undefined;
    if (allowKeep &&
...
export function applyPrimaryModel(cfg, model) {
    ...
    const fallbacks = typeof existingModel === "object" && existingModel !== null && "fallbacks" in existingModel
...
export function applyModelAllowlist(cfg, models) {
    const defaults = cfg.agents?.defaults;
