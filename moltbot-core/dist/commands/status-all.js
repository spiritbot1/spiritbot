...
export async function statusAllCommand(runtime, opts) {
    await withProgress({ label: "Scanning status --all…", total: 11 }, async (progress) => {
        ...
        const tailscale = await (async () => {
            try {
                ...
                const ips = self && Array.isArray(self.TailscaleIPs)
                    ? self.TailscaleIPs
        ...
        const tailscaleHttpsUrl = tailscaleMode !== "off" && tailscale.dnsName
        ...
        progress.tick();
        const defaultWorkspace = agentStatus.agents.find((a) => a.id === agentStatus.defaultId)?.workspaceDir ??
        ...
        const overviewRows = [
            ...
            nodeService
                ? {
                    Item: "Node service",
                    Value: nodeService.installed === false
