export function applyChannelMatchMeta(result, match) {
...
export function resolveChannelMatchConfig(match, resolveEntry) {
    if (!match.entry)
        return null;
    return applyChannelMatchMeta(resolveEntry(match.entry), match);
}
export function normalizeChannelSlug(value) {
    return value
        .trim()
        .toLowerCase()
        .replace(/^#/, "")
...
export function buildChannelKeyCandidates(...keys) {
    const seen = new Set();
    const candidates = [];
...
export function resolveChannelEntryMatch(params) {
...
export function resolveChannelEntryMatchWithFallback(params) {
