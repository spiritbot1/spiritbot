...
export const signalMessageActions = {
    listActions: ({ cfg }) => {
        ...
        const reactionsEnabled = configuredAccounts.some((account) => createActionGate(account.config.actions)("reactions"));
        if (reactionsEnabled) {
            actions.add("react");
        }
    ...
    handleAction: async ({ action, params, cfg, accountId }) => {
        ...
        if (action === "react") {
            ...
            const recipientRaw = readStringParam(params, "recipient") ??
