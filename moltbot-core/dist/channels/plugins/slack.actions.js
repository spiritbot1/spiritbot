...
export function createSlackActions(providerId) {
    return {
        listActions: ({ cfg }) => {
            ...
            const isActionEnabled = (key, defaultValue = true) => {
                for (const account of accounts) {
                    const gate = createActionGate((account.actions ?? cfg.channels?.slack?.actions));
        ...
        handleAction: async (ctx) => {
            ...
            const resolveChannelId = () => resolveSlackChannelId(readStringParam(params, "channelId") ?? readStringParam(params, "to", { required: true }));
            if (action === "send") {
            ...
            if (action === "member-info") {
                ...
                return await handleSlackAction({ action: "memberInfo", userId, accountId: accountId ?? undefined }, cfg);
            }
